...
func TestNewResourceBasicProperties(t *testing.T) {
	...
	if loggerResource.Type() != reflect.TypeOf((*ddd.Logger)(nil)) {
		t.Errorf("Expected resource type to be '%s', got '%s'", reflect.TypeOf((*ddd.Logger)(nil)).Elem(), loggerResource.Type())
	}
